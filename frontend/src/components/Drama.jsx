...
// import {drama} from "../assets/data.json"
...
useEffect(()=>{
  async function fetchmovies(){
    ...
    const fetched=data.data.filter((item)=>item.genre.includes("Drama"))
    setMovies(fetched)
  }
  fetchmovies()
...
async function addlist(item){
  ...
  return (
    <div className='relative my-8 mx-4'>
      <div className='text-2xl my-4'>Drama</div>
    ...
    <button onClick={handlePrev} className="absolute left-4 z-30  top-1/2 -translate-y-1/2 bg-black/50 hover:bg-black/70 text-white rounded-full p-3 backdrop-blur-md"
      ...
      {Movies && Movies.map((item,index)=>{
         const isOpen = showOverlay === index;
          return  <div key={index} className="relative w-[200px] h-[230px] shrink-0 hover:scale-105 group" onClick={() => setShowOverlay(isOpen ? null : index)}>

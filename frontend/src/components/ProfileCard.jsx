...
const ProfileCard = () => {
  ...
  const { name, email,setlogin, setname, setemail } = useContext(AppContext);
  ...
  return (
    <div className="mt-24 max-w-lg w-full bg-[#1e1e1e] text-white rounded-2xl shadow-lg border border-[#ff0000] p-8 mx-auto">
      <div className="flex flex-col items-center">
        <img src="https://i.pravatar.cc/150?img=3" alt="profile" className="w-28 h-28 rounded-full border-4 border-[#ff0000] mb-4" />
        <h2 className="text-3xl font-semibold mb-2">{name || "Guest User"}</h2>
      ...
      <div className="space-y-4 mt-6">
        <div>
          ...
          {editMode ? (
            ...
            <p className="bg-[#2b2b2b] rounded-md p-2 text-gray-200">{name}</p>
          )}
        </div>
        ...
        <div>
          ...
          {editMode ? (
            ...
            <p className="bg-[#2b2b2b] rounded-md p-2 text-gray-200">{email}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-end gap-4 mt-6">
        {editMode ? (
          ...
          <button  onClick={() =>{ setlogin(false);navigate('/login')}} className="bg-[#ff0000] hover:bg-[#ff3333] text-white px-4 py-2 rounded-md transition">
            Logout
          </button></>
        )}
      </div>
